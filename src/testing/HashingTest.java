...
public class HashingTest extends TestCase {
    ...
    public void testHashRangeWithBoundaryValues() {
        BigInteger atStart = ECSNode.RING_START;
        BigInteger atEnd = ECSNode.RING_END;
        assertTrue(ECSNode.isKeyInRange(atStart, ECSNode.RING_START, ECSNode.RING_END));
        assertTrue(ECSNode.isKeyInRange(atEnd, ECSNode.RING_START, ECSNode.RING_END));
    }

    public void testInHashRangeWithInvertedBoundaries() {
        BigInteger start = new BigInteger("F".repeat(32), 16);
        BigInteger end = new BigInteger("1".repeat(32), 16);
        BigInteger hash = new BigInteger("0".repeat(32), 16);
        assertTrue(ECSNode.isKeyInRange(hash, start, end));
    }

    public void testHashRangeWithSameStartAndEnd() {
        BigInteger startAndEnd = new BigInteger("A".repeat(32), 16);
        BigInteger key = startAndEnd;
        assertTrue(ECSNode.isKeyInRange(key, startAndEnd, startAndEnd));
    }
